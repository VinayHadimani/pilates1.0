    ...
    <main className="min-h-screen bg-black text-[#E1E0CC]">
      ...
      <header className="sticky top-0 z-30 border-b border-white/10 bg-black/80 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 md:px-6">
          <div className="flex items-center gap-3">
            <span className="flex h-9 w-9 items-center justify-center rounded-full bg-[#DEDBC8]">
              <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none">
                <path d="M2 14c3-6 6-6 9 0s6 6 11 0" stroke="#0a0a0a" strokeWidth="2.4" strokeLinecap="round" />
              </svg>
            </span>
            <div>
              <p className="text-sm font-bold uppercase tracking-[0.15em]">Arcwave Admin</p>
          ...
          <div className="flex items-center gap-2">
            <a
              href="/"
              target="_blank"
              className="inline-flex items-center gap-1.5 rounded-full border border-white/15 px-3 py-1.5 text-xs text-primary/70 transition-colors hover:text-primary"
            >
            ...
            <Button
              onClick={logout}
              variant="outline"
              className="rounded-full border-white/15 px-3 text-primary/70 hover:bg-white/5 hover:text-primary"
            >
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 md:px-6">
        ...
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          ...
          <Stat label="Est. membership revenue" value={formatINR(revenue)} />
        </div>

        <Tabs defaultValue="pricing" className="mt-8">
          <TabsList className="flex h-auto w-full flex-wrap justify-start gap-1 rounded-2xl bg-[#0e0e0e] p-1.5">
...
function TabTrigger({
  ...
  return (
    <TabsTrigger
      value={value}
      className="rounded-xl text-xs data-[state=active]:bg-primary data-[state=active]:text-black"
    >
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-[#0e0e0e] p-4">
      ...
      <p className="mt-2 text-2xl font-semibold text-[#DEDBC8]">{value}</p>
    </div>
  );
}
